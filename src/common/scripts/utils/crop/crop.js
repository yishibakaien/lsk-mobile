 ...
 */
import Transform from './AlloyTransform';
...
(function() {
    var AlloyCrop = function(option) {
        ...
        if (option.width !== option.height && option.circle) {
            throw 'can not set circle to true when width is not equal to height';
        }
        ...
        // 这里 添加类名（设置样式）
        this.cancel_btn.className = 'button button-red button-block';

        this.ok_btn = document.createElement('button');
        this.ok_btn.innerHTML = this.ok_text;
        // 这里 添加类名（设置样式）
        this.ok_btn.className = 'button button-blue button-block';
